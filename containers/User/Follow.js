    ...
    render() {
        ...
        if (this.props.pageInfoData.loggedInUserDetails && this.props.pageInfoData.loggedInUserDetails.user_id == (this.state.item.user_id ? this.state.item.user_id : this.state.item.owner_id)) {
            ...
            if(!this.props.fromView){
                return  <a className={this.props.className ? this.props.className : "follow"} style={{ opacity: "0",display:this.props.hideButton ? "none" : "" }}  href="#">{this.props.title ? this.props.title : this.props.t("Followed")}</a>
            }else{
                return null
            }
        }
