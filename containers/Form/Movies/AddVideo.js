...
class AddVideo extends Component {
    ...
    uploadMedia = (e) => {
       ...
       var allowedExtensions = ['mp4','mov','webm','mpeg','3gp','avi','flv','ogg','mkv','mk3d','mks','wmv'];
        if (allowedExtensions.indexOf(extension) === -1) 
        {
            ...
            return false;
        }else if( parseInt(this.props.pageInfoData.appSettings['movie_upload_limit'])  > 0 && e.size > parseInt(this.props.pageInfoData.appSettings['movie_upload_limit'])*1000000){
        ...
        axios.post(url, formData, config)
            .then(response => {
                if (response.data.error) {
                    ...
                    this.setState({percentCompleted:0,processing:false,localUpdate:true, videoWidth: response.data.videoWidth, validating: false, videoId: response.data.videoId, success: true, videoTitle: response.data.name, videoImage: response.data.images[0] });
    ...
    onSubmit = model => {
        ...
        if (this.state.editItem) {
            ...
            formData.append("videoId", this.state.editItem.movie_video_id)
        }
    ...
    render() {
        ...
        if(!this.state.editItem){
            ...
            if(this.props.pageInfoData.levelPermissions['movie.embedcode'] == 1)
            ...
            if(this.state.type == "embed" && this.props.pageInfoData.levelPermissions['movie.embedcode'] == 1){
                validator.push({
                    key: "code",
                    validations: [
                        {
        ...
        let groupData1 = []
        let languages = []
        if(this.props.pageInfoData.spokenLanguage){
            languages.push({ key: 0, value: 0, label: "Please Select Language" })
            this.props.pageInfoData.spokenLanguage.forEach(lan => {
