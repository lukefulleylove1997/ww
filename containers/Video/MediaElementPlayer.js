...
class MediaElementPlayer extends Component {
    ...
    render() {
        ...
        if(this.state.video.status == 1 && resolutionsVideo.length > 0){
            ...
            mediaHtml = `<video id="${key}" width="100%" height="100%" ${(this.state.video ? ` poster=${this.props.imageSuffix+this.state.video.image}` : '')}
            ...
            mediaHtml = `<video id="${key}" width="100%" height="100%" ${(this.state.video ? ` poster=${image}` : '')}
                            ${(typeof this.props.showControls != "undefined" ? (this.props.showControls ? ' controls' : '') : ' controls' )}   preload="auto">
                            ${mediaBody}
                        </video>`;
        }
