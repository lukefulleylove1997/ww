...
class Index extends React.Component {
    ...
    liClicked(id, data, e) {
        ...
        console.log(jQuery(e.target).prop('nodeName'));
        if (jQuery(e.target).prop('nodeName') != "B") {
            if (data.type.indexOf('_comments_') > -1) {
                ...
                if ((data.object_type == "users" || data.object_type == "members")) {
                    ...
                    if(data['movies'])
                        Router.push(`/watch?videoId=${data['movies'].custom_url}`, `/watch/${data['movies'].custom_url}`)
    ...
    let notiData = ""
        if (this.state.notifications && this.state.notifications.length) {
            notiData = this.state.notifications.map(notification => {
                ...
                const pattern = /(\{subject\})|(\{audio\})|(\{videos\})|(\{movies\})|(\{channels\})|(\{blogs\})|(\{playlists\})|(\{comment\})|(\{reply\})|(\{comment_title\})|(\{reply_title\})|(\{members\})|(\{planName\})|(\{channel_posts\})|(\{period\})/g;
                
                let image = null
                if (notification.object_type == "channels") {
                    if(notification['channels'])
                        image = <Links className="prevent content-image" data={notification["channels"]}><Image className="prevent" title={notification["channels"].title} image={ notification["channels"].image} imageSuffix={this.props.pageInfoData.imageSuffix} /></Links>
                } else if (notification.object_type == "blogs") {
                    if(notification['blogs'])
                        image = <Links className="prevent content-image" data={notification["blogs"]}><Image className="prevent" title={notification["blogs"].title} image={ notification["blogs"].image} imageSuffix={this.props.pageInfoData.imageSuffix} /></Links>
                } else if (notification.object_type == "artists") {
                    if(notification['artists'])
                        image = <Links className="prevent content-image" data={notification["artists"]}><Image className="prevent" title={notification["artists"].title} image={ notification["artists"].image} imageSuffix={this.props.pageInfoData.imageSuffix} /></Links>
                } else if (notification.object_type == "playlists") {
                    if(notification['playlists'])
                        image = <Links className="prevent content-image" data={notification["playlists"]}><Image className="prevent" title={notification["playlists"].title} image={ notification["playlists"].image} imageSuffix={this.props.pageInfoData.imageSuffix} /></Links>
                } else if (notification.object_type == "videos") {
                    if(notification['videos'])
                        image = <Links className="prevent content-image" data={notification["videos"]}><Image className="prevent" title={notification["videos"].title} image={ notification["videos"].image} imageSuffix={this.props.pageInfoData.imageSuffix} /></Links>
                }else if (notification.object_type == "movies") {
                    if(notification['movies'])
                        image = <Links className="prevent content-image" data={notification["movies"]}><Image className="prevent" title={notification["movies"].title} image={ notification["movies"].image} imageSuffix={this.props.pageInfoData.imageSuffix} /></Links>
                } else if (notification.object_type == "audio") {
                    if(notification['audio'])
                        image = <Links className="prevent content-image" data={notification["audio"]}><Image className="prevent" title={notification["audio"].title} image={ notification["audio"].image} imageSuffix={this.props.pageInfoData.imageSuffix} /></Links>
                }
