...
class Index extends React.Component{
        ...
        axios.post(url, formData, config)
            .then(response => {
                if(response.data.error){
                    ...
                    this.setState({submitting:false,live:true,custom_url:response.data.custom_url,video_id:response.data.videoId,image:response.data.image,title:response.data.title,allow_chat:model["enable_chat"],scheduled:model["scheduled"]})
                }
            }).catch(err => {
    ...
    render(){
        ...
        if(this.state.live){
            ...
            :
                streamingData = <MediaLiveStreaming scheduled={this.state.scheduled} streamToken={this.state.streamToken} streamType={this.state.streamType} streamingId={this.state.streamKey} {...this.props} allow_chat={this.state.allow_chat} title={this.state.title} image={this.state.image} video_id={this.state.video_id} custom_url={this.state.custom_url}  role="host" currentTime="0" allowedTime={parseInt(this.props.pageInfoData.levelPermissions["livestreaming.duration"]),10} />
        }
