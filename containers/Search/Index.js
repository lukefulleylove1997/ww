...
class Index extends React.Component{
    constructor(props){
        ...
        this.state = {
            fields:{
                ...
                latest:'Latest '+this.capitalizeFirstLetter(propsType+"s"),
                favourite:"Most Favourite "+this.capitalizeFirstLetter(propsType+"s"),
                view:"Most Viewed "+this.capitalizeFirstLetter(propsType+"s"),
                like:"Most Liked "+this.capitalizeFirstLetter(propsType+"s"),
                dislike:"Most Disliked "+this.capitalizeFirstLetter(propsType+"s"),
                commented:"Most Commented "+this.capitalizeFirstLetter(propsType+"s"),
                rated:"Most Rated "+this.capitalizeFirstLetter(propsType+"s"),
                played:"Most Played "+this.capitalizeFirstLetter(propsType+"s")
            },type:{
                featured:"Featured "+this.capitalizeFirstLetter(propsType+"s"),
                sponsored:"Sponsored "+this.capitalizeFirstLetter(propsType+"s"),
                hot:"Hot "+this.capitalizeFirstLetter(propsType+"s")
            },
            filterForm:0
        }
    }
    ...
    submitForm = (e) => {
        ...
        }else if(this.state.fields.genre){
            if(this.state.fields.q)
                values["genre"] = this.state.fields.q
        }else{
            if(this.state.fields.q)
                ...
                <React.Fragment>
                        <div className="grid-menu justify-content-between search-form">
                            <form className="row gy-3" onSubmit={this.submitForm.bind(this)}>
                                {
                                    this.state.fields.genre ? 
                                    <div className="form-group col-sm-4">
                                        <label htmlFor="q" className="control-label">{Translate(this.props,"Genre")}</label>
                                        <input type="text" onChange={this.changeTitle} value={this.state.fields.genre} id="q" className="form-control" placeholder={Translate(this.props,"Genre")} />
                                    </div>
                                :
                                        <div className="form-group col-sm-4">
                                ...
                                }
                                {
                                    ...
                                    <React.Fragment>
                                        {
                                            this.props.pageInfoData.spokenLanguage && this.props.pageInfoData.spokenLanguage.length > 0 ? 
                                                <div className="form-group col-sm-4">
                                                    <label htmlFor="name" className="control-label">{Translate(this.props,"Languages")}</label>
                                                    <select onChange={this.changeLanguage} className="form-control form-select" value={this.state.fields.language}>
                                                        <option value="">{Translate(this.props,"Select Language")}</option>
                                                        {
                                                            this.props.pageInfoData.spokenLanguage.map(res => {
                                                                return (
                                                                    <option key={res.code} value={res.code}>{Translate(this.props,res.name)}</option>
                                                                )
                                                            })
                                                        }
                                                    </select>
                                        ...
                                        {
                                            this.props.pageInfoData.countries && this.props.pageInfoData.countries.length > 0 ?  
                                                <div className="form-group col-sm-4">
                                                    <label htmlFor="name" className="control-label">{Translate(this.props,"Countries")}</label>
                                                    <select onChange={this.changeCountry} className="form-control form-select" value={this.state.fields.country}>
                                                        <option value="">{Translate(this.props,"Select Countries")}</option>
                                                        {
                                                            this.props.pageInfoData.countries.map(res => {
                                                                return (
                                                                    <option key={res.id} value={res.id}>{Translate(this.props,res.nicename)}</option>
                                                                )
                                                            })
                                                        }
                                                    </select>
