...
class Trailers extends React.Component{
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        ...
        } else if (nextProps.pageInfoData.movie != prevState.movie || prevState.episode != nextProps.episode) {
           return {
                ...
                <div key={item.movie_video_id} className="gridColumn">
                    <div className="ThumbBox-wrap">
                        ...
                        this.state.episode && this.state.seasons? 
                            <Link href="/watch" customParam={`trailerId=${item.movie_video_id}&videoId=${this.state.movie.custom_url}&seasonId=${this.state.episode.season}&episodeId=${this.state.episode.episode_number}`} as={`/watch/${this.state.movie.custom_url}/season/${this.state.episode.season}/episode/${this.state.episode.episode_number}/trailer/${item.movie_video_id}`}>
                                {html}        
                            </Link>
                        :
                            <Link href="/watch" customParam={`trailerId=${item.movie_video_id}&videoId=${this.state.movie.custom_url}`} as={`/watch/${this.state.movie.custom_url}/trailer/${item.movie_video_id}`}>
                                {html}        
                            </Link>
                        }
                    </div> 
