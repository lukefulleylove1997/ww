...
exports.goLive = async(req,res) => {
    ...
    await globalModel.custom(req, "SELECT owner_id FROM videos WHERE video_id = ?", videoId).then(async result => {
        if (result && result.length) {
    ...
    globalModel.custom(req, "UPDATE videos SET scheduled = null,is_livestreaming = 1 WHERE video_id = '"+videoId+"'").then(result => {
    ...
    notificationModel.insertFollowNotifications(req,{subject_type:"users",subject_id:req.user.user_id,object_type:"videos",object_id:videoId,type:"livestreaming_live"}).then(result => {

    }).catch(err => {

    })

    notifications.insertScheduledFollowers(req, {  subject_type: "users", subject_id: videoObject.owner_id, object_type: "videos", object_id: videoId, type: "videos_reminder" }).then(result => {
    ...
    // dataNotification["object_id"] =  videoId
    ...
    dataScheduledNotification["object_id"] =  videoId
...
exports.create = async (req, res) => {
    ...
    if (videoId) {
        ...
        await globalModel.update(req, insertObject, "videos", 'video_id', videoId).then(async result => {
            ...
            //}
            res.send({editItem:videoObj,approved:videoObj ? videoObj.approve : "", videoId: videoId, message:videoObject['custom_url'] ?  constant.video.EDIT : constant.video.SUCCESS, custom_url: videoObject['custom_url'] ? videoObject['custom_url'] : insertObject['custom_url'] });
            if(req.body.fromEdit){
                videoId = null
                res.end()
                return
        ...
        await globalModel.create(req, insertObject, "videos").then(async result => {
            if (result) {
                ...
                res.send({editItem:videoObj,approved:videoObj ? videoObj.approve : "", videoId: result.insertId, message: constant.video.SUCCESS, custom_url: insertObject['custom_url'],title:insertObject['title'],image:imagevideo });
            } else {
                return res.send({ error: fieldErrors.errors([{ msg: constant.general.DATABSE }], true), status: errorCodes.invalid }).end();
            }
        }).catch(err => {
