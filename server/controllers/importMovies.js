...
exports.importTitles = async (res,req,data,type,mdb) => {
        ...
        let orgData = {
            ...
            'description' :  data.overview ? data.overview : null,
            'language' : data.original_language ? data.original_language : null,
            'tagline' : data.tagline ? data.tagline : null,
            'budget' : data.budget ? data.budget : 0,
            'revenue' : data.revenue ? data.revenue : 0,
            'runtime' :  parseInt(exports.getRuntime(req,data,type,mdb)),
...
{
    return path ? PROFILE_BASE_URI + path : null;
}
