...
exports.approve = async (req, res) => {
    ...
    await globalModel.custom(req, "SELECT * from videos where video_id = ?", id).then(async result => {
        if (result && result.length) {
            let item = result[0]
            await globalModel.update(req, { approve: !item.approve }, "videos", "video_id", id).then(async result => {
                if (item.owner_id != req.user.user_id && !item.approve) {
                    ...
                    notifications.insert(req, {notChangeOwnerID:true, owner_id: item.owner_id, type: type, subject_type: "users", subject_id: item.owner_id, object_type: "videos", object_id: item.video_id, insert: true }).then(result => {
                ...
                if(!item.approve){
                    ...
                    if(item["type"] != 3 && !item["scheduled"]){
                        await notificationModel.insertFollowNotifications(req,{subject_type:"users",subject_id:item.owner_id,object_type:"videos",object_id:item.video_id,type:"members_followed"}).then(result => {
                    ...
                    if(parseInt(item.type) == 11 || parseInt(item.type) == 10){
                        if(!item['scheduled']){
                            ...
                            await notificationModel.insertFollowNotifications(req,{subject_type:"users",subject_id:item.owner_id,object_type:"videos",object_id:item.video_id,type:"scheduled_live"}).then(result => {
