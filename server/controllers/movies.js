...
exports.castView = async(req,res) => {
    ...
    req.query.cast = cast
    req.query.castncrewId = cast.cast_crew_member_id
...
exports.create = async (req, res) => {
    ...
    if(req.query.editItem){
        ...
        await movieModel.getPhotos(req,{resource_id:req.query.editItem.movie_id}).then(result => {
         ...
         req.query.videos = {}
         let limit = 51
         await movieModel.getVideos(req,{resource_id:req.query.editItem.movie_id,limit:limit}).then(result => {
...
exports.purchase = async (req, res) => {
    ...
    req.session.movieId = movie.movie_id 
...
exports.successul = async (req, res, next) => {
    ...
    if(id != req.session.movieId && (gateway != 2)){
        await commonFunction.getGeneralInfo(req, res, "page_not_found")
        req.app.render(req, res, '/page-not-found', req.query);
        return
    }
    ...
    if(gateway == "1" || !gateway){
        if (!req.user || !req.session.movietokenUserPayment || !req.session.movie_user_id || !req.session.movieId || !req.session.orderId) {
...
exports.category = async (req, res) => {
    ...
    let category = {}
    await categoryModel.findByCustomUrl({ id: req.query.categoryId, type: "movie" }, req, res).then(result => {
        if (result)
            category = result
    }).catch(error => {
        if (req.query.data) {
...
exports.view = async (req, res, fromOtherSource) => {
    ...
    let showMovie = true
    req.isview = true;
    await movieModel.getMovies(req, { custom_url: req.query.videoId ? req.query.videoId : "notfound", movieview: true }).then(result => {
        if (result && result.length > 0) {
            movie = result[0]
        } else {
            showMovie = false
    ...
    if(trailer_id){
        await movieModel.getVideos(req,{resource_id:movie.movie_id,limit:2,extraVideos:true,movie_video_id:trailer_id,season_id:fromOtherSource ? fromOtherSource.season_id : false,episode_id:fromOtherSource ? fromOtherSource.episode_id : false}).then(async result => {
            if(result && result.length > 0){
                ...
                req.query.trailerId = trailer_id
    ...
    if(season_id && episode_id){
        await movieModel.getVideos(req,{getFull:1,resource_id:movie.movie_id,limit:1,episode_id:fromOtherSource.episode_id,season_id:fromOtherSource.season_id}).then(async result => {
            ...
            if(result && result.length > 0){
                ...
                req.query.seasonId = season_id
                req.query.episodeId = episode_id
